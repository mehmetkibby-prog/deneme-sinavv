...
function renderHome(){
  ...
  $(".feature-grid").onclick=e=>{const b=e.target.closest("[data-go]");if(b)({teacher:renderTeacher,cards:renderFlashcards,study:renderStudy,profile:renderProfile}[b.dataset.go])()};
...
function renderVoice(){
  const recording=state.recorder?.state==="recording";setTitle("AI ile Sesli Çalış","Bas-konuş yöntemi",false);
  app.innerHTML=`<section class="hero"><h2>AI ile sesli çalış</h2><p>Düğmeye basıp sorunu söyle. Kaydı bitirdiğinde AI dinleyip sesli cevap verir.</p></section><div class="voice-orb ${recording?"live":""}">◉</div><div class="actions center"><button class="${recording?"danger":"primary"}" id="voice-toggle">${recording?"Kaydı Bitir":"Konuşmaya Başla"}</button></div><div id="voice-status" class="result">${recording?"Dinliyorum…":"Mikrofon izni istendiğinde izin ver."}</div><div id="transcript"></div>`;
  $("#voice-toggle").onclick=recording?stopRecording:startRecording;
}
...
async function processVoice(){
  const status=$("#voice-status");if(status)status.textContent="Ses yazıya çevriliyor…";state.recorder.stream.getTracks().forEach(t=>t.stop());
  try{
    const key=store.get("apiKey",""),blob=new Blob(state.chunks,{type:state.recorder.mimeType}),fd=new FormData();fd.append("file",blob,"konusma.webm");fd.append("model","gpt-4o-mini-transcribe");fd.append("language","tr");
    const tr=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${key}`},body:fd});if(!tr.ok)throw new Error((await tr.json()).error?.message||"Ses çözülemedi");
    const question=(await tr.json()).text;if(status)status.textContent="AI düşünüyor…";const answer=await openAIText(question);
    $("#transcript").innerHTML=`<div class="message me"><b>Sen:</b> ${esc(question)}</div><div class="message ai"><b>Öğretmen:</b> ${esc(answer)}</div>`;
    if(status)status.textContent="Sesli yanıt hazırlanıyor…";const sr=await fetch("https://api.openai.com/v1/audio/speech",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-4o-mini-tts",voice:"marin",input:answer,instructions:"Türkçe, sıcak ve öğretici bir öğretmen tonuyla konuş."})});if(!sr.ok)throw new Error((await sr.json()).error?.message||"Sesli yanıt alınamadı");
    state.audio=new Audio(URL.createObjectURL(await sr.blob()));await state.audio.play();if(status)status.textContent="Yanıt oynatılıyor. Yeniden konuşabilirsin.";
  }catch(e){if(status)status.textContent="Hata: "+e.message}finally{state.recorder=null}
}
